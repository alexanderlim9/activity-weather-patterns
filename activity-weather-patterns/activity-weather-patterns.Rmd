...
req <- fitbit_GET("1/user/4QXD3G/activities/steps/date/2016-06-23/2017-11-25.json", token = token )
...
weather2016 <-
  ...
  weatherFrame <-
  as.data.frame(cbind(
  as.numeric(weatherDate),
  as.numeric(weatherTempHi),
  as.numeric(weatherTempLo),
  as.numeric(weatherHumidity),
  as.numeric(weatherWind),
  as.numeric(weatherPrecip)
  ))
  colnames(weatherFrame) <-
  c("date", "tempHi", "tempLo", "avgHumidity", "avgWind", "precip")
  weatherFrame
```
